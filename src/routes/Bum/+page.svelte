		...
		session.subscribe(async ($session) => {
			if ($session.user) {
				...
				if (userCity) { // Check if userCity is available
					...
					const cityDocRef = doc(db, userCity, "feed", "posts", "msTGdhz7rQ633GW5Igxp");
					const cityDocSnap = await getDoc(cityDocRef);
					console.log("got from gainesville db");
					const postData = cityDocSnap.data();
					console.log("Post ID:", cityDocSnap.id);
